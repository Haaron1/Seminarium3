...
public class View {
    ...
    */
    public void runFakeExecution(){
        ...
        Receipt receipt = contr.pay(200, totalPrice);
        
        System.out.println("Time of sale: " + receipt.getSaleTime() + "\n" 
                + receipt.printPurchasedItems() 
                + "\nYour change: " + df.format(receipt.getChange()) 
                + "\nPaid amount: " + df.format(receipt.getPaidAmount())
                + "\nTotal price without tax: " + df.format(receipt.getTotalPriceWithoutTax())
                + "\nTotal price: " + df.format(receipt.getTotalPrice()) 
                + "\nTax amount: " + df.format(receipt.getTotalTax()) 
                + "\nTax rate: " + df.format(receipt.getTotalTaxRate()) + "%");
    }
}
